...
var type = {
  ...
  has: {
    index: function index(source, value) {
      ...
      if (type.is.folder(value)) {
        fs.readdirSync(path.join(source + value)).map(function (value) {
    ...
    children: function children(source, value) {
      ...
      if (type.is.folder(value)) {
        fs.readdirSync(path.join(source + value)).map(function () {
          hasChildren = true;
        });
      }
...
function createResrouce(dir, value, index, parent, root) {
  ...
  if (type.is.folder(value) && !type.is.item(dir, value)) {
    ...
    var indexContent = '';
    fs.readdirSync(dir).map(function (value) {
      if (/index..+$/.test(value)) {
  ...
  if (type.is.folder(value)) {
    ...
    fs.readdirSync(path.join(dir + value)).map(function (value, index) {
      if (!type.is.index(value)) {
        resource[_parent2].push(createResrouce(_subDir, value, index, _parent2, root));
      }
    });
  }
